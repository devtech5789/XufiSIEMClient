...
import EventViewerDump
import SecurityStance

LOGSERVER = "http://192.168.1.13:8000/logingestor/"
AGENTID = 2
headers = {'Content-Type': 'application/json'}


# SecurityStance - Collects security related information on OS
# EventViewerDump - Collects event logs from windows (System, Application, Security

securitySettings = {}

def healthCheckPing():
    while True:
        response = requests.get("http://192.168.1.13:8000/healthcheck/?agentid=" + str(AGENTID), headers=headers)
        print(response.text)
        time.sleep(30)        
